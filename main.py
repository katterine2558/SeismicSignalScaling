...
def generar_ventana_1():
    ...
    #Ícono ventana
    ventana_nivel_1.after(201, lambda :ventana_nivel_1.iconbitmap(os.path.join(images_path, "logo.ico")))
    #Label
...
def generar_ventana_2():
    ...
    #Ícono ventana
    ventana_nivel_2.after(201, lambda :ventana_nivel_2.iconbitmap(os.path.join(images_path, "logo.ico")))
    #Listbox
...
def generar_ventana_3():
    ...
    if len(sismos_seleccionados) == 0:
        ...
        #Ícono ventana
        ventana_nivel_3.after(201, lambda :ventana_nivel_3.iconbitmap(os.path.join(images_path, "logo.ico")))
        #Label
...
def write_files(sismos:dict):
    ...
    os.mkdir(f"{ruta_documentos}\input_deepsoil/02_consolidado/escalado_componente/")
    ...
    for sismo in list(sismos.keys()):
        ...
        #Escribe el sismo por componente y escalado con el número de pasos y el delta de tiempo
        componentes_sismo = list(sismos[sismo].keys())
        for index_componente in range(len(componentes_sismo)):
            with open(f"{ruta_documentos}\input_deepsoil/02_consolidado/escalado_componente/{sismo}-{componentes_sismo[index_componente]}.txt", 'w') as f:
                f.write(f"{len(sismos[sismo][componentes_sismo[index_componente]]['vector'])}\t{sismos[sismo][componentes_sismo[index_componente]]['tiempo'][1]}\n")
                for line in lines:
                    tiempo = line[0].split("\t")[0]
                    accel = line[index_componente + 1].split("\t")[0]
                    f.write(f"{tiempo}\t{accel}\n")
            f.close()
...
def generar_ventana_4():
    ...
    #Ícono ventana
    ventana_nivel_4.after(201, lambda :ventana_nivel_4.iconbitmap(os.path.join(images_path, "logo.ico")))
    #Label
...
def compilar_output_deepsoil():
    ...
    def ejecutar_compilar_resultados_deepsoil():
        ...
        for f in motion_name:
            ...
            compilado_output_deepsoil[f] = {}
            for hoja in hojas:
                if "Layer" in hoja or "Input Motion" == hoja:
                    
                    #Inicializa el almacenamiento
                    compilado_output_deepsoil[f][hoja] = []
                    #Hoja seleccionada
                    sheet = wb[hoja]
                    #Letra para buscar fila máxima
                    if "Layer" in hoja:
                        fila_ind ="J"
                        min_row = 2
                        min_col , max_col = 10 , 10
                    else:
                        fila_ind ="E"
                        min_row = 3
                        min_col , max_col = 5 , 5

                    max_fila = encontrar_maxima_fila_convalor(sheet,fila_ind)

                    # Iterar sobre la hoja para extraer el PSA
                    for fila in sheet.iter_rows(min_row=min_row, max_row=max_fila, min_col=min_col, max_col=min_col):
                        for celda in fila:
                            compilado_output_deepsoil[f][hoja].append(celda.value)
        ...
        #Encuentra el nombre de las hojas (coge el primero como referencia)
        sismos = list(compilado_output_deepsoil.keys())
        hojas = list(compilado_output_deepsoil[sismos[0]].keys())
        ...
        for hoja in hojas:
            ...
            for j in range(len(sismos)):
                name_signal = sismos[j] #nombre del sismo
                ws.write(0,j,name_signal,formato_centrado)
                for i in range(len(compilado_output_deepsoil[name_signal][hoja])):
                    ws.write(i+1,j,float(compilado_output_deepsoil[name_signal][hoja][i]),formato_centrado)
